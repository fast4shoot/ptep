Material getColorForPoint(vec4 position, vec3 normal, vec2 uv)
{
	...
	float mix0 =  smoothstep(-0.01, 0.01, snoise(vec3(uv * 14, 465)));
	float mix1 = smoothstep(-0.01, 0.01, snoise(vec3(uv * 16, 12)));
	float mix0123 = smoothstep(-0.01, 0.01, snoise(vec3(uv * 16, 54)));
